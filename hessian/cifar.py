...
parser = argparse.ArgumentParser(description='Hessian of CIFAR')
...
parser.add_argument('-n', '--num_hidden',    help='Size of hidden layer',   type=int, default = 64)
parser.add_argument('--hessian_num_batches',    help='Hessian batches',   type=int, default = 8)
...
opt = {
    ...
    'l2' : 1e-2,
    'scale' : 1e-1,
    'network' : [256*3, args['num_hidden'], 10],
...
n, tx, ty, vx, vy = load_cifar('/local2/pratikac/cifar/cifar-10-batches-py', opt, dtype)
p = init_params(opt['scale'], opt['network'], npr.RandomState(args['seed']))
