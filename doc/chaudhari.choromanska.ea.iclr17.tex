...
\todo{Let us note that tuning the momentum hyper-parameter was crucial to getting good results on RNNs. While the SGD baseline on $\ptblstm$ does not use momentum and in fact does not train well with momentum, we set it to $0.5$ for $\entropysgd$. On the other hand, the baseline for $\charlstm$ was trained with Adam with a $\b_1 = 0.9$ and we reduced this to $\b_1 = 0.5$ for $\entropyadam$.}
