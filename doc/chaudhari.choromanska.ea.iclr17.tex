...
\textbf{Christian Borgs$^{5}$, Jennifer Chayes$^{5}$, Riccardo Zecchina$^{4}$}\\[0.05in]
...
%[This is incorrect -- k-SAT has different behavior.] In discrete optimization problems, such wide local minima are dense clusters of solutions in the parameter space where nearby parameters have similar energy, say, the number of clauses violated in $k$-SAT~\citep{sipser2006introduction,achlioptas2006solution}.
...
%We will set the inverse temperature $\b$ to $1$ because $\g$ affords us similar control on the Gibbs distribution.

\begin{definition}[Local free entropy]
...
\begin{algorithm}[H]
    ...
    \HyperParameters{\quad $\textrm{Scope}\ \g$, $\textrm{Data weight}\ \rho$, $\textrm{Learning rate}\ \eta$}
    \vspace{0.1in}
